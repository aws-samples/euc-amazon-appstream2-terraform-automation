...
module "appstream_image_builder" {
  ...
  image_name           = local.image_builder_base_image_name
  instance_type        = local.image_builder_instance_type
...
module "appstream_fleet" {
  ...
  fleet_type                         = local.fleet_type
  desired_fleet_instances_count      = local.desired_fleet_instances_count
  max_user_duration_in_seconds       = local.fleet_max_user_duration_in_seconds
  disconnect_timeout_in_seconds      = local.fleet_disconnect_timeout_in_seconds
  idle_disconnect_timeout_in_seconds = local.fleet_idle_disconnect_timeout_in_seconds
  image_name                         = local.fleet_image_name
  instance_type                      = local.fleet_instance_type
  security_group_cidrs               = [for s in data.aws_subnet.routable_front_end : s.cidr_block]
  name_prefix                        = local.name_prefix
  default_tags                       = local.default_tags
}
